...
| BaselineInterp | `baseline_interp` | **Ubica bien las direcciones NO-exactas del relevamiento anterior** (geocodebr `numero_aproximado` apila muchos números en un punto, y a veces ese punto está a **km de la calle real** — validado vs OSM). Estrategia en 4 niveles por confiabilidad: (1) **Mapbox por número** (solo resultados `address`/`street`; **descarta los `place`/ciudad** que aterrizarían en el centro — usa `match_code.confidence`; descarta apilados ≥3=centroide y fuera de zona 120 km; `mapbox`); (2) **geometría de calle de OSM** (Overpass 1 consulta/calle por token + núcleo fuzzy ≥0.82; `osm_interp`); (3) **Gemini** canoniza el nombre de calle no matcheado (ej. "R ORIEL B CAMPOS"→"Rua Oriel Bezerra de Campos", **cacheado** en `calle_canonica` mig. 038, no re-paga; los 429 NO se cachean) y reintenta Mapbox/OSM; (4) lo que ninguna ubica → **centro de la ciudad, marcado** `geocode_source='ciudad'` (la web lo dibuja con borde ámbar punteado "≈ ubicación aproximada", no inventa la cuadra). **No pisa** un `osm_interp`/`interp` previo bueno si Overpass flakea. Respeta las exactas `g:numero`. **Corre solo** como pase final de **BaselineGeocoder** y por RPC. El match de nombre usa el **núcleo** (sin tipo de vía ni basura "(LOT …)"). Compeer: 342 mapbox + 83 osm + 200 exactas + 15 ciudad. |
...
HotelFetcher manda un **🆘 aviso por Telegram con un link** a la página `/asistencia-hoteles/{survey}`
(operador): mapa con solo los hoteles + ficha (teléfono/dirección/situação/fuente) + carga manual
