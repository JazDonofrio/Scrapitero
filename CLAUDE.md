...
| Agente | RPC | Cuándo usarlo |
|--------|-----|---------------|
| VGPipelineRunner | `vg_pipeline_runner` | **VG: PREFERIDO.** Happy path compilado: SmartGIS→BCI(parse inline)→Parser→Agrupador→**HotelFetcher**→**HotelHabitacionesLLM** en UNA llamada determinista. Los dos pasos de hoteles son **opcionales/best-effort** (no tumban el relevamiento si fallan); hoteles usa fuentes gratis (sin Google). El survey **no se marca `completed`** hasta terminar TODOS los pasos. Registra pasos en `surveys.notes`, honra stop. `parcial:true` ⇒ re-invocar (continúa). Los agentes de abajo quedan para correr pasos sueltos/debug |
...
| ParcelaCategoria | `parcela_categoria` | **Aterriza los establecimientos CNPJ sobre las parcelas relevadas.** Para una región: ubica los establecimientos geocodificados de `receita_estabelecimentos` dentro de cada parcela (`ST_Contains`, acotado al bbox de las parcelas) y **sella** `parcelas.categoria_uso` (R/C/E) + `descripcion_uso` (BAR, ESCOLA, HOSPITAL PÚBLICO…, mig. 029). Una parcela con varios establecimientos lista todas; la categoría es la de mayor peso (E>C>R). Idempotente (resetea los sellos `receita_cnae` de la región antes de re-aplicar). Insumo del **tipo de edificación unificado** de la web. |
...
industrial→INDÚSTRIA, comercial→COMÉRCIO EM GERAL, mixto→MIXTO. Se muestra en el popup de la
...
y comparar: **Dirección completa**, **Tipo de inmueble** (uso) y **Ciudad** (columna del CSV,
por fila — autodetectada). **La ciudad NO se pide en el formulario** (ni la fecha): sale de
la columna mapeada y se agrega a cada dirección al geocodificar (sin ciudad caerían en
cualquier parte del país); el backend deriva además una **ciudad predominante** como fallback
y para detectar el país. Al confirmar, el backend crea la región + persiste el baseline y
