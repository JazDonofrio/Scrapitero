...
| OsmPoiFetcher | `osm_poi_fetcher` | **POIs de OSM que Overture NO publica (GRATIS).** Completa —no reemplaza— a `OverturePlacesFetcher`: los `tags` son un parámetro para que el próximo rubro vacío se sume acá y no en otro agente. Hoy: **`amenity=fuel` (estaciones de servicio)**, medido en Malvinas el 12-ago-2026 — Overture devolvió **0 en 218 POIs** (ni un `gas_station`, ni un YPF/Shell/Axion/Puma por nombre) en una zona cruzada por las rutas 8 y 202, y **OSM tenía 3 en el bbox, 2 dentro del polígono**, una de ellas la que el operador ve al lado del McDonald's de Illia. Pide **nodos Y ways** (el playón suele ser un `way`; `out center` da el centroide) y recorta al polígono exacto. Escribe `comercios` (`source='osm'`, `place_id='osm:way/123'`) con el **rubro nombrado igual que en Overture** (`gas_station`) para que los filtros por rubro sigan valiendo, y `establecimientos_poi` con **`fuente='osm_poi'`** — NO `'osm'`: `ShoppingFetcher` borra sus POIs por fuente y se los llevaría puestos. **Reusa la guarda de huella de `OverturePlacesFetcher._link_to_parcelas`** (importada, no copiada). Si Overpass no contesta confiable devuelve **`ok:false`**, nunca 0 POIs: en la corrida real hicieron falta 4 mirrors (406 + 504 + un 200-vacío rechazado + el cuarto con los datos). Después correr `ParcelaCategoria`, que le pone el **piso de UF=1** al lote sin conteo |
...
Argentina usa la **huella de los footprints** en vez del área construida.
...
- **`OverturePlacesFetcher` es el que marca comercio** (gratis) y **manda sobre el catastro**:
  su sello está en `USO_FUENTES_PROTEGIDAS`, así que un `residencial` inferido del conteo
  nunca degrada el `mixto`/`comercial` que puso quien sí vio los comercios. El uso sale de
  **las dos UF de la fila**, no del `uso_principal` previo — con el CASE viejo, que sólo
  ascendía a `mixto` si la parcela ya venía `residencial`, en Hurlingham **27 de 28**
  parcelas con vivienda Y comercio terminaron `comercial` y ninguna mixta.
