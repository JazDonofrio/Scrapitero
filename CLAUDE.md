...
| Agente | RPC | Cuándo usarlo |
|--------|-----|---------------|
| ARBACartoFetcher | `arba_carto_fetcher` | **PBA: SIEMPRE primero.** Requiere JSESSIONID. Si falla login → Telegram al usuario. Si no hay parcelas en DB las baja de IDERA por **filtro espacial** (polígono de la zona) o por nomenclatura si se pasa completa. Cuenta las subparcelas (≥25 m² → UF, menores → cochera) y carga la **UF total** en `uf_vivienda` **y en `uf_catastro`** (mig. 056), con `uf_fuente='arba_carto'`: ARBA **no dice el destino** (el campo `sp` es el número de subparcela, no el uso), así que el reparto vivienda/comercio lo hacen después las fuentes de POI **descontando** de ese total — ver **Regla PBA**. ⚠ La diferencia entre las dos columnas es lo que hace que el descuento sea idempotente: **`uf_vivienda` es una interpretación** que las fuentes de comercio corrigen, **`uf_catastro` es el crudo** que no toca nadie más. Y ojo con lo que NO arregla: donde no hay ninguna señal de comercio, la unidad sin destino se sigue contando como vivienda — medido en Malvinas, **1.875 de 1.974 parcelas con exactamente "1 vivienda"** puesta por default. Es la decisión que falta acordar con el cliente: contarla como vivienda o dejarla sin clasificar. |
...
| GooglePlacesFetcher | `google_places_fetcher` | Comercios de Google Maps (cualquier país). **Después de UnidadesEstimator.** Baja POIs comerciales por teselas adaptativas, los vincula a parcela (`ST_Contains`) y aporta el **conteo real de `uf_comercio`** (cada comercio = +1 UF, `uf_fuente='google'`) + señal de uso (parcela con comercio → comercial/mixto, `uso_fuente='google'`). Caro (~USD 0,032/req): tope `max_requests` + Telegram. **Fuera de Brasil preferir `OverturePlacesFetcher`, que da lo mismo gratis** |
| OverturePlacesFetcher | `overture_places_fetcher` | **Comercios y equipamiento GRATIS (Overture Maps Places).** La alternativa libre a GooglePlaces y **la fuente de comercios fuera de Brasil**, donde no hay padrón fiscal descargable (ARCA sólo consulta por CUIT con clave fiscal ⇒ `ReceitaEstabFetcher` no tiene equivalente). GeoParquet sobre **S3 público consultado con DuckDB — sin token, sin credenciales y sin costo por request** (mismo patrón que `FootprintFetcher`). Licencias **CDLA-Permissive-2.0** (Meta/Microsoft) y **Apache-2.0** (Foursquare): uso comercial permitido. Recorta al polígono de la zona, escribe `comercios` (`source='overture'`), vincula por `ST_Contains`, aporta **`uf_comercio` = conteo real** (`uf_fuente='overture'`) + señal de uso, y carga en `establecimientos_poi` los que mapean a la taxonomía del cliente (ESCOLA/HOSPITAL/SHOPPING/SUPERMERCADO…) para que **`ParcelaCategoria`** los aterrice y el tipo de edificación sea específico. Lee **`basic_category`**, no `categories` (deprecada, **se elimina en el release 2026-09**). ⚠ **`operating_status` viene NULL** ⇒ no sirve como señal de abierto/cerrado, no se filtra por él. El bucket **no permite listar releases**: la versión es un parámetro (`release`, default `2026-07-22.0`). Guarda de precedencia contra otras fuentes pero **no contra sí mismo** (si no, no podría refrescar su conteo con un release nuevo). **Guarda de huella (`exigir_huella`, default on)**: el punto de Overture viene corrido unos metros, así que un POI que cae en un lote **sin ninguna construcción** se reasigna al lote **construido** más cercano (≤ `reasignar_max_m`=40 m) y, si no hay ninguno, queda sin parcela — un lote vacío no puede tener un comercio adentro. Necesita `FootprintFetcher` corrido antes; sin footprints la guarda se saltea con WARNING (*"no hay edificio" ≠ "no se bajaron los edificios"*). Caso que la motivó (Malvinas, ago-2026): un «Burger King» que en su propia ficha dice *BK Terrazas de Mayo Shopping* quedó dentro de una **plaza de 8.022 m² sin un edificio**, a 27 m del lote del shopping, aportándole una UF de comercio que salió al CSV del cliente. **Guarda de número (`exigir_numero`, default on)**: la de huella sólo rescata al POI que cayó en un lote **vacío**; cuando el lote equivocado también tiene edificios, el único testigo es **la dirección que el POI declara en su propia ficha** (ver `candidatos_numero_ajeno`, la implementación única que comparten el fetcher y el reporter). Se mueve solo **un caso y uno solo**: misma calle (`_misma_calle`, estricto sobre `normalizar_calle` + tolerancia para el nombre de pila omitido —`José Darragueira` ⊃ `Darragueira`—, **nunca `nucleo_calle`**, que fusiona vías distintas) y **salto de `salto_min`=300 números** contra el de su parcela, habiendo una parcela **construida** que sí lleve ese número a ≤ `numero_max_m`=60 m. Una cuadra argentina son 100 números: un punto corrido unos metros no puede explicar un salto de 300. Caso testigo (Malvinas, ago-2026): *Questa Pizza* declara **Illia 3770** y estaba parada en **Illia 30** —el lote del McDonald's—, a 7 m del lote 3770 de 131.620 m² que es el shopping. ⚠ **Todo lo demás NO se toca y va al panel como `poi_numero_ajeno`**, porque medirlo dio vuelta la intuición: de 137 POIs con dirección, 65 declaran un número distinto al de su parcela, 14 tienen una parcela cercana con ese número y **sólo 1 llega a moverse**. Los dos grupos que quedan para ojo humano: (1) **esquina** — calle declarada ≠ calle del catastro; parece el señalón más fuerte y es el más traicionero, **3 de 4 tenían una parcela de la calle declarada pegada (≤5 m)**, o sea un lote de dos frentes que el catastro rotula por uno y el comercio publicita por el otro (mismo modo de falla que las etiquetas de calle del BCI); (2) **vecino** — misma calle, número contiguo (Artigas 171 declarando Artigas 161, con la 161 a 4 m): no se puede saber si el punto está corrido o si el número del catastro está mal, y moverlo hace bailar filas del CSV del cliente sin ganancia verificable. Los saltos del grupo `vecino` van de **7 a 57** y el de Questa Pizza es **3.740**: el umbral cae en una banda vacía enorme, no está calibrado contra un caso único. ⚠ Esta guarda corrige **de qué lote cuelga el comercio, no la dirección del lote**: Illia 30 sigue saliendo al CSV con ese número, que en una calle numerada 3770-4651 es casi seguro un rótulo malo del catastro. Y **no cubre al POI sin número parseable**: *Starbucks* declara «Cruce Ruta 8 Y 202 San Miguel» y se queda donde está. **Las dos guardas necesitan `FootprintFetcher`** corrido antes; sin footprints se saltean con WARNING. ⚠ **`_agregar_uf` DESCUENTA, no suma, y cuenta TODAS las fuentes de POI** (`FUENTES_POI` = Overture + OSM), sellando **`uf_fuente='poi'`** (el sello `'overture'` quedó como histórico; seguir usándolo declaraba mal el origen en el CSV, el mismo modo de falla que las parcelas argentinas selladas `'cadastur'`). **Descuento**: donde el catastro dio un conteo **sin destino** (`parcelas.uf_catastro`, mig. 056 — ARBA no publica el uso de la subparcela), los comercios confirmados **no son unidades nuevas**: son parte de ese mismo total mal rotulado como vivienda ⇒ `uf_vivienda = max(uf_catastro − comercios, 0)` y `UF total = max(uf_catastro, comercios)`. Es lo que `UsoClassifier` documenta desde el principio (*"lo que confirma se DESCUENTA del total de ARBA, no se suma encima"*) y esta ruta hacía **al revés**: el shopping Terrazas de Mayo salía con **«1 vivienda + 32 comercios»** y el lote del McDonald's con «1 vivienda + 2 comercios». Medido en Malvinas al aplicarlo: mixtas **77 → 11**, comerciales **2 → 68**, y el edificio real de José León Suárez 1800 conserva sus 82 UF repartidas (77 viv + 5 com) en vez de inflarse a 87. **La resta se calcula siempre desde `uf_catastro`, nunca desde el `uf_vivienda` ya restado** — si no, cada corrida restaría de nuevo; verificado idempotente en 4 pasadas. En Brasil `uf_catastro` queda **NULL** (el BCI sí declara el destino de cada unidad) y ahí el comportamiento no cambia. Además **barre las parcelas que dejaron de tener comercios** (release nuevo o POI desvinculado): vuelven a `uf_comercio=0`, la vivienda **vuelve al crudo de `uf_catastro`** (la otra mitad de la idempotencia) y el uso al que corresponde sin comercio, para que el sello no firme una UF sin nada detrás. Medido en Hurlingham: **299 POIs en el bbox, 100% con dirección**; 137 dentro de la zona → 53 UF de comercio en 28 parcelas y 78 con etiqueta del cliente |
| HotelFetcher | `hotel_fetcher` | **Hoteles del relevamiento (multi-fuente, `fuentes=['cadastur','receita','osm','google']` por default).** **Cadastur** (oficial Brasil: nombre, CNPJ, **habitaciones/UHs**, leitos, tipo, situação; **lee primero la tabla local `cadastur_hospedagem`** que carga [[project_cadastur_local]] (`CadasturLocalFetcher`) y solo cae al portal CKAN si esa ciudad no está cacheada) + **Receita** (universo CNPJ de hospedagem + **situação cadastral**, dedupe por CNPJ con Cadastur, **mucha más cobertura**, sin habitaciones; coordenadas gratis vía `GeocodebrFetcher`/CNEFE, fallback a nominatim/Google para las que falten) + **OSM** (`tourism=hotel…`, gratis y siempre arriba; `rooms`/`stars` cuando están tageados) + **Google Places** (`lodging`, **descubrimiento PAGO** ~USD 0,032/req, hasta 80 teselas adaptativas; aporta ubicación + abierto/cerrado vía `businessStatus`, **no** trae habitaciones/UHs). El botón "🏨 Hoteles" de la web corre con este default (incluye Google) — para evitar el costo, invocar el agente con `fuentes=['cadastur','osm']`. Geocodifica los sin coords (Cadastur/Receita) con **geocodebr primero** (CNEFE/IBGE, gratis, en lote vía `geocode_forward`; tope de desvío `geocodebr_max_desvio_m`=300 m), y solo lo que no ubique cae a **Nominatim→Mapbox→Google** (Mapbox pago barato si hay `MAPBOX_TOKEN`); recorta a la zona, dedupe entre fuentes y **vincula a parcela** (`ST_Contains`). Enriquece **abierto/cerrado** con el `business_status` de Google (de `comercios`): `cerrado_def` = situação Inativo/Cancelado (Cadastur) **o** Receita `BAIXADA`/`NULA` **o** `CLOSED_PERMANENTLY` (Google). Para hoteles **abiertos**, suma sus **habitaciones como `uf_comercio`**, los cerrados no cuentan. ⚠ **El sello depende de si el número es un CONTEO o un PISO** (mig. 061): `uf_fuente='cadastur'` sólo cuando TODOS los hoteles de la parcela traen habitaciones reales (padrón/OSM `rooms`/carga manual); si alguno no las trae, su aporte es el `GREATEST(COALESCE(habitaciones,1),1)` —«hay un hotel acá», sin saber cuántos cuartos— y va **`hotel_min`**. Antes sellaba siempre `cadastur`, que **fuera de Brasil es directamente falso**: Cadastur es el padrón brasilero, en Argentina no existe y el pipeline nunca lo consultó ni podía. Medido: 4 parcelas de Malvinas (y **18 de Várzea Grande**, así que no era sólo un problema argentino) declaraban un origen imposible. El daño no es el rótulo: `cadastur` está en `UF_FUENTES_PROTEGIDAS` porque significa *conteo oficial exacto*, así que el piso quedaba blindado con la autoridad de un registro estatal y ninguna fuente mejor podía corregirlo. Mismo modo de falla que `'overture'` → `'poi'`. La migración **no cambia ningún número**, sólo el sello. ⚠ **Y el UPDATE final ya no pisa `manual`**: el uso sí lo respetaba, la UF **no**, así que una parcela con hotel corregida a mano volvía al conteo automático en la corrida siguiente sin aviso (2 casos en Malvinas, entre ellos la escuela militar de Lemos). ⚠ `bci_parser` tiene la lista de precedencia **copiada a mano**: se le agregó `hotel_min`, pero sigue sin proteger `overture`/`poi` — no se unificó con la constante porque incluye `'bci'` y el parser se auto-bloquearía. **Habitaciones — prioridad:** exacto de Cadastur (UHs) > OSM (`rooms`) > **manual** (asistencia humana, `hotel_habitaciones_manual`, mig. 030 + **059**) > **IA Gemini** (`HotelHabitacionesLLM`) > **estimación por área del BCI** (`area_m2_construida ÷ m2_por_habitacion`, default 35) — la estimación BCI está **DESACTIVADA por default** (`estimar_habitaciones=False`; daba disparates) y tiene tope/guarda de área. El origen queda en `habitaciones_fuente` (`cadastur`/`osm` exacto, `manual`, `llm`, `bci_proxy` estimado); la UI marca **"(IA)"** / **"≈ N (estimado)"**. Dedupe cross-fuente sin CNPJ por **dirección normalizada o nombre + proximidad escalonada** (Google no trae CNPJ): la **puntuación se saca al comparar** (Google devuelve `Hotel "El Mesidor"` y OSM `El Mesidor`: venía pegada al token —`{'"el','mesidor"'}`— y el mismo hotel entraba dos veces; `_norm` NO se toca, que también normaliza headers de CSV de Cadastur y nombres de município); nombre FUERTE (igual/núcleo distintivo idéntico) permite radio amplio (`merge_dist_fuerte_m`=2500 m — las coords de Cadastur/Receita son la dirección fiscal geocodificada, lejos del pin real), nombre débil exige ≤`merge_dist_m`=200 m; compartir UNA palabra ya NO matchea (bug "Amazon"). **CNPJs distintos ya no separa** (re-registro del mismo hotel): fusiona si misma dirección o mismo nombre fuerte ≤200 m. **Dedupe cross-run**: las filas ya en DB de fuentes que no corren (p.ej. Google paga de una corrida anterior) entran como semillas y se fusionan en vez de duplicarse. En el merge, la **coordenada y dirección física del pin Google/OSM pisan la geocodificada** (la dirección fiscal de Receita confundía a la asistencia). El enriquecimiento de `business_status` por comercios cercanos (60 m) ahora exige además **nombre similar**. El `cerrado_def` de la fusión **sigue a la fuente ganadora** (ya no es un OR acumulativo: si quedó cerrado por un falso positivo viejo de Google, Cadastur/Receita más nuevo y confiable lo puede reabrir). El `DELETE` previo al reinsert y el `INSERT` (con `ON CONFLICT` sobre `uq_hoteles_region_cnpj`) respetan el mismo scope de survey que las semillas, para no robarle hoteles a otro survey de la región ni romper la corrida si dos surveys comparten un CNPJ. El `uf_comercio` que aporta un hotel a su parcela se **resetea** cuando el hotel cerró o se reubicó a otra parcela entre corridas (si no, quedaba pegado con el valor viejo, y un hotel reubicado terminaba contado en dos parcelas a la vez). ⚠ **El respaldo manual de habitaciones no servía FUERA de Brasil** (arreglado el 14-ago-2026, mig. 059). `hotel_habitaciones_manual` tenía PK `(region_id, cnpj)` con `cnpj NOT NULL`, y el re-apply aparejaba con `h.cnpj = m.cnpj`. En Argentina **no existe identificador fiscal del hotel** —no hay padrón hotelero descargable y ni Google ni OSM lo publican—, así que los dos lados quedaban NULL y `NULL = NULL` no es TRUE: el apareo no matcheaba nunca y el override ni siquiera se podía guardar. Como `run` **borra y reinserta** las filas de `hoteles` en cada corrida, el número que el operador cargaba a mano se perdía **en silencio** — sin error, sin warning, sin nada en el log; la tarjeta volvía a decir «sin habitaciones» y parecía que no se había guardado. Medido en Malvinas: **7 hoteles, los 7 sin CNPJ, 0 overrides recuperables**. Ahora `cnpj` es nullable con **dos índices únicos parciales** —`(region_id, cnpj)` donde hay CNPJ (**Brasil idéntico a antes**) y `(region_id, nombre_norm)` donde no— y el re-apply son dos pasos: los CON CNPJ en SQL como siempre, los SIN CNPJ en **Python** (`_reaplicar_habitaciones_sin_cnpj`), por **nombre normalizado + ≤200 m**, el mismo criterio que `_esta_descartado`. Se resuelve en Python y no en SQL a propósito: así el nombre pasa por `_norm`, la única implementación, en vez de una traducción de acentos escrita a mano en SQL que se desincronizaría y rompería el match sin avisar. Las tres copias del INSERT que había en `app.py` quedaron unificadas en `_guardar_habitaciones_manual`. ⚠ **`hotel_cerrado_manual` tenía el MISMO bug y se arregló igual** (mig. 060, `_reaplicar_cerrado_sin_cnpj`). Ahí perder el override es **peor que perder un dato**: un hotel que el operador cerró y que la fuente sigue dando ATIVA **revive en cada corrida y vuelve a aportar `uf_comercio` a su parcela**, o sea se mete solo en el entregable. El apareo sin CNPJ lo comparten las dos tablas en **`_parear_overrides_sin_cnpj`** — un solo lugar donde vive el criterio (nombre normalizado + ≤200 m). Al sumar un override nuevo de hotel: **si la clave es el CNPJ, fuera de Brasil no existe**; usar ese helper. Capta **teléfono** (Receita) para la asistencia humana. Cuando Cadastur vuelva, el dato exacto pisa la estimación al re-correr. Escribe `hoteles` (mig. 025-026), idempotente (borra+reinserta por fuente; conserva `fuente='demo'`). Si una fuente falla (Cadastur 502) sigue con las otras; solo falla si **todas** caen. Un **cron de Hermes** (`hermes cron`, job `cadastur-watch`, cada 4 h, script `scripts/cadastur_watch_hermes.py`) chequea el portal CKAN y avisa por Telegram en cada **transición** (caído→disponible y disponible→caído, con el código HTTP: 502 backend caído, 403/451 bloqueo por IP, timeout); deja historial en `/opt/data/cadastur_watch.log`. Ver memoria [[project_hermes_cron]]. |
...
| ParcelaCategoria | `parcela_categoria` | **Aterriza los establecimientos CNPJ + POIs sobre las parcelas relevadas.** Para una región: ubica los establecimientos geocodificados de `receita_estabelecimentos` **union** los POIs de `establecimientos_poi` (shoppings de OSM/Google) dentro de cada parcela (`ST_Contains`, acotado al bbox de las parcelas) y **sella** `parcelas.categoria_uso` (R/C/E) + `descripcion_uso` (BAR, ESCOLA, HOSPITAL PÚBLICO, SHOPPING…, mig. 029). ⚠ **Sólo empresas que siguen en pie** (20-ago-2026): se descartan las `situacao` **BAIXADA y NULA**, que son cierre definitivo — `INAPTA`/`SUSPENSA` NO lo son y siguen sellando, mismo corte que `HotelFetcher`. El sello nació como **clasificador de uso de suelo**, donde daba igual si el CNAE seguía activo; después `_tipo_edificacion` le dio prioridad sobre todo (es la etiqueta del mapa) y `DXFEntrega` lo copió en `TIPOIMOVEL`: un clasificador ascendido a evidencia sin revisar el supuesto. Medido en VG: **74 de 239 parcelas selladas lo estaban sólo por empresas dadas de baja** (102 sumando inaptas/suspensas), y **BAIXADA es el 49% del dump de MT**. Verificado en la calle en Av. Couto Magalhães 352, que salía `LANCHONETE` por un bar dado de baja **declarado en el 374** —el geocoding lo corrió 12 m al lote vecino— mientras la verdulería que sí está ahí no figura en ninguna fuente. Una parcela con varios lista todas; la categoría es la de mayor peso (E>C>R). Idempotente (resetea los sellos `receita_cnae` de la región antes de re-aplicar). Insumo del **tipo de edificación unificado** de la web. ⚠ **Cómo aterriza depende de la fuente, y esto fue un bug grande.** Los POIs de un fetcher **con guardas** (`OverturePlacesFetcher`, `OsmPoiFetcher`) traen el vínculo **ya resuelto** en `establecimientos_poi.parcela_id` (mig. 057) y **manda ése, no la geometría**. Antes se aterrizaba TODO por `ST_Contains` sobre la coordenada cruda, así que las guardas de huella y de número corregían `comercios.parcela_id` y **`ParcelaCategoria` las deshacía en silencio** volviendo a poner la etiqueta donde el POI caía. Medido en Malvinas (12-ago-2026): **38 parcelas con etiqueta de comercio y `uf_comercio=0`** —el popup decía «🏢 RESTAURANTE» y abajo «Com: 0»—, entre ellas «**Calle Juan» rotulada LANCHONETE por el Burger King** que ya vivía en el lote del shopping: *el fantasma que la guarda de huella daba por corregido seguía saliendo al CSV por esta puerta*. Otras enganchaban por la **tolerancia de 5 m** a la parcela vecina teniendo el comercio bien puesto al lado, y otras etiquetaban lotes cuyo comercio la guarda había dejado **sin parcela** por no tener una sola construcción. Con el fix: **38 → 0**, 87 etiquetas → 66 (las 21 que se fueron eran todas falsas). `vinculo_resuelto=true` con `parcela_id` NULL significa "una guarda lo desvinculó a propósito" y **no aterriza en ningún lado** — sin esa bandera el fallback geométrico lo volvía a poner justo donde la guarda lo sacó. Los POIs sin guardas (Receita, shoppings de `ShoppingFetcher`) siguen por geometría. ⚠ **El sello `manual` de `categoria_uso_fuente` se respeta en el UPDATE, no sólo en el reset** (13-ago-2026). El reset de idempotencia borraba únicamente lo sellado `receita_cnae` —bien— pero el UPDATE final **no filtraba por fuente** y pisaba igual la corrección del operador. Medido en Malvinas: la parcela del **Círculo de Suboficiales** (…3700C), corregida a mano a `INSTITUICAO ESPORTIVA` después de verla en campo, volvió a salir **LANCHONETE** por el buffet del club — el mismísimo POI que había motivado la corrección. El 🏢 del popup no se notaba porque `parcela_tipo_manual` tiene prioridad 0, pero el 🏷️ y el `descripcion_uso` del CSV sí salían mal. Al tocar este agente: **el reset y el UPDATE tienen que filtrar por la MISMA condición de fuente**. |
| IncidenciasReporter | `incidencias_reporter` | **Junta en un solo lugar los casos que sólo un humano puede resolver** y los carga en `incidencias` (mig. 046), que alimenta la página **`/incidencias/{survey_id}`**. Antes cada señal moría en un log, un aviso de Telegram o una página de un solo tipo — **la asistencia de hoteles quedó absorbida** como el tipo `hotel_sin_habitaciones`. Tipos: **`geocoding_dudoso`** (dirección del relevamiento anterior mal ubicada — **el más urgente**: con esos puntos se define la zona, así que un error ahí hace que se releven las parcelas equivocadas. Dos señales por geometría: `parcela_ajena` = la coordenada cae dentro de una parcela de otra calle · `lejos_de_su_calle` = está a más de `lejos_calle_m` (150 m) de **toda** parcela de su propia calle. **No** marca caer *entre* parcelas: `catastro_interp` interpola entre anclas y aterrizar en la vía es normal — con el criterio ingenuo salían 163 casos en VG de los cuales 144 eran eso; midiendo distancia a su calle quedan **44** reales. Excluye las fuentes exactas `catastro`/`g:numero`) · **`numero_faltante`** (el catastro no trae el número de puerta y `NumeroEstimator` **se negó a estimarlo** —calle con numeración incoherente o sin anclas—: en VG 26 casos, casi todos en `CLOVIS HUGNEY` 0,42 · `JOAO LIBANIO` 0,41 · `MAL RONDON` 0,55 · `SÃO BERNARDO` 0,56. La tarjeta trae los **vecinos con número** como referencia y el operador carga la altura mirando el frente en Street View; queda en **`parcela_direccion_manual`** (mig. 052, clave `(region_id, cca_code)` para **sobrevivir al re-scrape** — el `parcela_id` cambia entre surveys, la inscrição no) y se escribe en `parcelas.numero` con `direccion_source='manual'`) · **`hotel_sin_habitaciones`** (hotel abierto sin UHs en ninguna fuente) · **`altura_sin_declarar`** (el catastro no declara construcción y el satélite ve un edificio — el más valioso) · **`altura_mas_alta`** (satélite ve más pisos que el proxy del catastro) · **`uf_imposible`** (la UF declarada no cabe en el volumen visible: `(huella × pisos) / uf_vivienda < m2_por_uf_min`, default 25 m²/UF; detecta errores de carga del **baseline Y del BCI** — el umbral vive en el reporter, no en `altura_fetcher`, para no tocar el criterio de una capa ya corrida) · **`uf_sin_declarar`** (el **espejo** del anterior: hay construcción y **CERO** unidades declaradas, o sea la parcela sale del relevamiento aportando 0 UF. **Dos vías de evidencia, según qué publique el catastro:** (1) **área construida** —Brasil/BCI—, donde **no necesita satélite** porque la contradicción está dentro del propio dato y no depende de la imagen (en VG es de 2014 en el 89% de los puntos): **13 casos**, el peor `SAO BENTO 156` con **703 m² construidos sobre 519 m² de terreno** —ratio 1,36, o sea más de una planta— y `uso='vacante'`; (2) **huella de `footprints_revision`** donde el catastro NO publica superficie construida —**ARBA la deja en NULL**, así que fuera de Brasil el caso era invisible aunque estuviera a la vista—: en Malvinas Argentinas **23 casos**, el mayor con 9 edificios dentro del lote. La huella se **recorta a la parcela** (`ST_Intersection`, solape mín. 10 m²) y **no** se atribuye por centroide: un galpón que cubre varios lotes le daba sus 901 m² enteros a una parcela de 206 m² (ratio 4,37, un imposible que hace desconfiar del caso), y además los lotes vecinos que ese mismo galpón cubre quedaban afuera. Filtro `area_construida_min` = 30 m² para descartar galpón/garaje sin unidad propia, aplicado a las dos vías. Cuando hay medición de altura se adjunta a la tarjeta como corroboración, pero no hace falta para detectar) · **`poi_numero_ajeno`** (el comercio está parado en una parcela cuyo número **no es el que él mismo declara**, y a pocos metros hay otra construida que sí lo lleva. La detección la comparte con `OverturePlacesFetcher.candidatos_numero_ajeno` —una sola implementación— y **acá llegan sólo los casos que la guarda automática NO se anima a mover**: `esquina` (lote de dos frentes, lo más probable es que esté bien donde está) y `vecino` (número contiguo, indistinguible de un punto corrido). Prioridad 3 y `descartar` como acción de primera clase: en Malvinas son **11 casos** y la lectura es que la mayoría está correcta. Mientras la incidencia esté pendiente **el dato no se toca** y el POI sigue aportando su UF donde está). **Idempotente preservando el trabajo humano**: upsert por `(survey_id, tipo, clave)` con **clave natural** estable entre corridas (`<tipo>:<parcela_id>`; para hoteles `hotel:<cnpj|nombre_norm>` — el `hotel_id` NO sirve, `HotelFetcher` borra+reinserta). El `ON CONFLICT` refresca el contenido del caso pero **nunca** pisa `estado`/`resolucion`/`nota`/`autor`, así re-correr 🏨 o 📏 no reabre lo resuelto; las pendientes cuya condición ya no se cumple pasan a **`obsoleta`** (y si reaparecen se reabren). Botón **"🧾 Incidencias"** (`POST /api/surveys/{sid}/incidencias/generar`) + banner en la tarjeta del survey con el desglose por tipo. Ver memoria [[project_incidencias]]. |
...
| FootprintFetcher | `footprint_fetcher` | **Capa de REVISIÓN visual de footprints de edificios — NO toca `parcelas`/`edificios` ni el relevamiento.** Objetivo: comparar contra lo que dice el catastro (BCI en Brasil, exacto pero puede quedar desactualizado si hubo una construcción no declarada) un footprint independiente derivado de imagen satelital. Prioridad de fuente por bbox: **1) Google Open Buildings** — vía el mirror de **VIDA** en FlatGeobuf (`source.coop/vida/google-microsoft-open-buildings/flageobuf/by_country/country_iso=<ISO3>/<ISO3>.fgb`, republica el dataset de Google partido por país con columna `bf_source` que permite filtrar solo `'google'`), consultado con **DuckDB** (`httpfs`+`spatial`, ya dependencias del proyecto; requiere `s3_region='us-west-2'` + `s3_url_style='path'` — el estilo virtual-host rompe TLS por los puntos del bucket) usando el **índice espacial de FlatGeobuf** (no descarga el país completo, ~5-15 s por bbox). País (ISO-3) autodetectado del centroide del bbox con `geo.detect_country` — genérico para cualquier región del mundo, no hardcodeado a Brasil. **2) OSM (fallback)** si el archivo no existe para ese país o no hay resultados en el bbox — reusa los helpers de `osm_building_fetcher.py` (Overpass). Escribe `footprints_revision` (mig. 044, tabla separada de `edificios` para no contaminar `UnidadesEstimator` en Salta/PBA) idempotente por survey (borra+reinserta) y vincula a parcela por `ST_Contains` **filtrando por `region_id`** (necesario: varias regiones de VG se superponen geográficamente). **Además materializa `parcelas.huella_m2`** (mig. 058, `_calcular_huella_m2`): m² de solape parcela↔footprint, `ST_Union` antes de medir (dos huellas de la misma fuente pueden pisarse) y umbral de **25 m² por huella** para descartar el roce de digitalización del edificio del vecino — mismo criterio que las guardas de `OverturePlacesFetcher`. Es la única excepción al "no toca `parcelas`", y es deliberada: el cálculo espacial cuesta ~700 ms por relevamiento y `_tipo_edificacion` lo pide desde el mapa, el panel, el DXF y el CSV. **Escribe 0, no NULL, en la parcela sin huella** — el NULL queda reservado para "este survey no bajó footprints", que es otra afirmación. **No es paso de `VGPipelineRunner`** — queda standalone a propósito, para revisar antes de decidir si hace falta actuar. Botón **"🏗️ Footprints (revisión)"** (in-process, `POST /api/surveys/{sid}/footprints`) + capa toggleable del mapa (`GET .../footprints` → GeoJSON, solo footprints vinculados a parcela; popup compara área/confidence del footprint vs. `uf_vivienda`/tipo de edificación del catastro). Ver memoria [[project_footprint_fetcher]]. |
| CountryFetcher | `country_fetcher` | **Barrios cerrados / condomínios (loteamentos fechados) desde OSM.** No salen del catastro/CNPJ (son un ÁREA que contiene muchas parcelas, no un POI). Una consulta Overpass por áreas con `residential=gated` o **nombre** de condomínio/loteamento fechado dentro de la zona (`COALESCE(subzona, zone_geojson)`); construye los polígonos de los **anillos cerrados** (una vía abierta "Rua Condomínio X" no forma polígono → se descarta, así mantiene precisión) y **estampa `parcelas.es_country`** (mig. 042) de las parcelas cuyo centroide cae adentro (shapely). Idempotente por survey/región (resetea el flag antes), best-effort (si Overpass cae no rompe, re-ejecutable). ⚠ **El `name_regex` era sólo brasilero y fuera de Brasil no encontraba NADA** (13-ago-2026): en Argentina lo mismo se llama *country*, *barrio cerrado*, *barrio privado* o *complejo cerrado*, no *condomínio*. Medido en Malvinas: **0 áreas con `residential=gated` y 0 nombres brasileros** en todo el polígono — y sin embargo el **Complejo Las Horquetas** estaba ahí, con seguridad 24 h, 3 piletas y dos torres, metido en la tabla `hoteles` porque **Google publica los barrios cerrados con `primaryType=lodging`**. Ése es un modo de falla nuevo y caro: no es un comercio mal tagueado (el caso «Casa Cortina»), es una urbanización entera contada como hotel y con sus parcelas rotuladas comercial. ⚠ Y **ni con el regex arreglado sale solo**: OSM no tiene mapeado el complejo como área, así que sus dos parcelas se marcaron a mano. Fuera de Brasil, `es_country` hay que mirarlo a ojo — el agente evita el próximo caso, no rescata éste. Alimenta la **capa 🏘 Country** del mapa. Botón "🏘 Country" (in-process, `POST /api/surveys/{sid}/country`) + paso del `VGPipelineRunner` tras `parcela_categoria`. |
| OsmPoiFetcher | `osm_poi_fetcher` | **La SEGUNDA fuente de comercios, gratis (OSM).** Completa —no reemplaza— a `OverturePlacesFetcher`: los `tags` son un parámetro para que el próximo rubro vacío se sume acá y no en otro agente. **Desde ago-2026 el default son los ~88 tags de `_TAGS`** (shop/amenity/office/craft → taxonomía del cliente), no sólo las estaciones. Por qué: Overture **no veía 32 negocios que OSM sí** —Supermercado Luna, Autoservicio Nelly, 3 carnicerías, 4 restaurantes, ferreterías, Maderera Burger…— y esas parcelas salían al CSV como `residencial` con `uf_comercio=0`, que es exactamente lo que el operador nota al comparar contra Google Maps. Aplicado en Malvinas: **26 parcelas cambiaron de uso** (22 residencial→comercial), comerciales **68 → 91**. Aporta `uf_comercio` con la MISMA `_agregar_uf` de Overture (que cuenta las dos fuentes). ⚠ **Dedupe contra las otras fuentes ANTES de insertar** (`_descartar_duplicados`), o el mismo negocio cuenta dos veces: **sólo por NOMBRE, nunca por cercanía sola** — medido, hay **21 pares de negocios DISTINTOS a menos de 20 m** («Il Cappo» a 5 m de «Kiosko San Miguel»), que es lo normal en una tira de locales, y deduplicar por proximidad tiraría 21 comercios reales. Radio **escalonado por lo distintivo del nombre**: 600 m con 2+ tokens significativos (el mismo criterio y valor que `shopping_fetcher.merge_dist_fuerte_m` — *Terrazas de Mayo Shopping* aparece a 175 m entre fuentes porque OSM apunta al centro del polígono y Overture a una tienda ancla) y **250 m con un solo token**, porque un nombre de pila no identifica un negocio (*«Elizabeth»*, dos peluquerías **distintas** a 675 m). Los 10 pares reales de Malvinas resuelven bien con esa regla. También **barre los POI de OSM que ya no corresponden** (desaparecieron del mapa, o una corrida vieja los guardó y el dedupe de hoy los descarta): sin eso un duplicado colado queda inflando `uf_comercio` para siempre. ⚠ **El dedupe también matchea por CONTENCIÓN, no sólo por núcleo idéntico** (13-ago-2026). `_nombre_fuerte` exige que los conjuntos de tokens distintivos sean **iguales**, y eso dejaba pasar el mismo negocio escrito con una palabra de más: «Terrazas de Mayo» (OSM) vs «Terrazas de Mayo Shopping» (Overture) a 157 m, y «Clínica Neuropsiquiatrica San Miguel» vs «Clínica Privada Neuropsiquiátrica Día San Miguel» **a 0 m** — dos veces la misma UF en el CSV. Se arregló en dos capas, y **medir cambió el diseño dos veces**: (1) las palabras que no distinguen se agregaron a `_GENERICOS_NOMBRE` (`shopping`, `srl`, `sas`, `sac`, `scs`), con lo que *Terrazas de Mayo Shopping* y *Maderera Burger SRL* caen solas por la vía de igualdad que ya existía — **`sa` NO se agregó: se come el apellido «Sá»**, y `S.A.` con puntos ya caía por el filtro de longitud; (2) `_nombre_contenido` (señal DÉBIL) para el resto. Pedir sólo contención es **peligroso**: medido, un POI de OSM llamado literalmente **«San Miguel»** —el partido vecino— se comía *Carrefour Hipermercado San Miguel*, *Kiosko San Miguel*, *Frávega san miguel 2* y 5 más. Por eso la contención exige además **≤ 25 m** y **match ÚNICO en todo el radio**: si el nombre está contenido en VARIOS negocios distintos no identifica a ninguno — *la ambigüedad es la señal*, y evita necesitar un padrón de topónimos (no serviría: `localidad`/`municipio` vienen **vacíos** en PBA y el partido vecino tampoco sale del nombre de la región). Sin el tope de 25 m «Terrazas de Mayo» se comía a sus propios locales —*LOCAL 47*, *Destel*, el *Patio de comidas*, a 109-153 m—. Resultado sobre los datos reales: **5 fusiones correctas, 0 falsas**, y 0 hoteles afectados. Antecedente que originó el agente: **`amenity=fuel`**, medido en Malvinas el 12-ago-2026 — Overture devolvió **0 en 218 POIs** (ni un `gas_station`, ni un YPF/Shell/Axion/Puma por nombre) en una zona cruzada por las rutas 8 y 202, y **OSM tenía 3 en el bbox, 2 dentro del polígono**, una de ellas la que el operador ve al lado del McDonald's de Illia. Pide **nodos Y ways** (el playón suele ser un `way`; `out center` da el centroide) y recorta al polígono exacto. Escribe `comercios` (`source='osm'`, `place_id='osm:way/123'`) con el **rubro nombrado igual que en Overture** (`gas_station`) para que los filtros por rubro sigan valiendo, y `establecimientos_poi` con **`fuente='osm_poi'`** — NO `'osm'`: `ShoppingFetcher` borra sus POIs por fuente y se los llevaría puestos. Un tag **sin equivalente claro** en la taxonomía (p.ej. `shop=yes`, "acá hay un negocio y no sé de qué") **cuenta la UF pero NO sella** `establecimientos_poi`: mismo criterio que `_clasificar` en Overture, no se le inventa una etiqueta. **Reusa las guardas de `OverturePlacesFetcher._link_to_parcelas`** (importadas, no copiadas). La consulta a Overpass **agrupa los tags por clave con regex**: con un `node`+`way` por tag el set completo son ~180 sentencias y devuelve **504** (medido, dos mirrors seguidos); agrupado son 10 y contesta en segundos. Si Overpass no contesta confiable devuelve **`ok:false`**, nunca 0 POIs: en la corrida real hicieron falta 4 mirrors (406 + 504 + un 200-vacío rechazado + el cuarto con los datos). ⚠ **El último recurso es Tor y estaba muerto** (arreglado el 13-ago-2026): `_try_mirrors` en `osm_building_fetcher` —el helper que usan TODOS los agentes de Overpass— pasaba `proxies=` a `httpx.Client`, argumento que httpx 0.28 renombró a `proxy=`. Como el proxy sólo se usa cuando los mirrors directos fallan, en vez de rescatar la corrida la reventaba con un `TypeError` justo en el momento en que era la única salida, y nadie lo notaba porque el camino feliz nunca lo toca. Después correr `ParcelaCategoria`, que le pone el **piso de UF=1** al lote sin conteo |
| ShoppingFetcher | `shopping_fetcher` | **Shoppings reales (no salen del CNPJ).** Receita NO identifica shopping centers (el CNAE 6822 es "administração de propriedade imobiliária" = todas las inmobiliarias → remapeado a IMOBILIÁRIA). Los trae de **OSM `shop=mall`** (gratis) + **Google Places `shopping_mall`** (pago) + **`shoppings_ar`** (directorio curado [shoppings.com.ar](https://shoppings.com.ar/provincia-de-buenos-aires.html), **gratis, solo Argentina**, fuera del default para no pegarle al sitio en corridas brasileras). El directorio aporta el **nombre comercial real**, que OSM/Google suelen tener incompleto o mal tageado, y **no cuesta nada**: la ubicación sale de geocodificar con **georef-ar**, y lo que no tiene dirección postal —muchos están sobre accesos de ruta ("Panamericana Km 50")— se busca **por nombre en OSM validando con el reverse de georef que el punto caiga en el partido declarado**; si no cae, se descarta en vez de inventarle una coordenada. Su `robots.txt` permite el crawleo (`Disallow:` vacío) y se baja **una página por corrida**. Gotchas del parseo (Weebly): hay que preservar los `<br>` como saltos o todo queda en un renglón; la primera línea del bloque **no** es la dirección sino una variante del nombre comercial; y el ámbito se busca **excluyendo la línea de dirección**, porque muchas calles se llaman igual que un partido (*Norcenter, «Esteban Echeverria 3750»* en Vicente López daba el partido Esteban Echeverría, a 60 km). La validación compara contra el **partido**, no la localidad (Unicenter está en «Martínez», partido San Isidro). Medido: **16 de 23 ubicados**, el resto son accesos de ruta sin dirección postal. **No trae la cantidad de locales** — ese dato sigue sin resolverse en ninguna fuente. Recorta a la zona, dedupe por **nombre + proximidad ESCALONADA** como el de hoteles: nombre **fuerte** (idéntico o mismos tokens significativos) permite `merge_dist_fuerte_m`=600 m, porque un shopping ocupa una manzana y cada fuente lo apunta donde quiere (OSM el centro del polígono, Overture el POI de una tienda ancla) — *Terrazas de Mayo Shopping* entró dos veces con el nombre **idéntico** y a **175 m**, 25 m por encima del corte viejo; nombre apenas parecido exige `merge_dist_m`=150 m (así *Plaza Oeste* no se come a *Plaza Norte*); sin nombre en alguna fuente —típico de un nodo OSM sin tag `name`— cae a sola distancia con el radio corto. La **puntuación se saca al comparar** (`Shopping "Norcenter"` ≡ `Norcenter`): venía pegada al token y ni el núcleo ni difflib matcheaban. Carga `establecimientos_poi` (mig. 031) con categoria='E'/descripcion='SHOPPING'. **Dedupe cross-run**: las filas ya en DB de fuentes que no corren hoy (p.ej. Google de una corrida anterior) entran como semillas y se fusionan en vez de duplicarse — antes correr OSM y Google por separado (el patrón recomendado para el paso gratis del pipeline VG) duplicaba el mismo shopping. `ParcelaCategoria` los aterriza junto a los CNPJ, con **tolerancia de borde** (`ST_DWithin` 5 m como fallback de `ST_Contains`) para no perder un shopping cuyo punto cae unos metros afuera del polígono. Idempotente por región. **Paso del pipeline VG** (solo OSM gratis; para Google correrlo aparte con `fuentes=['google']`). **Ningún fuente gratuita da la cantidad de locales de un shopping real** (Receita no distingue, OSM/Google tampoco) — `ParcelaCategoria` le pone un **piso de UF=1** (`uf_fuente='shopping_min'`) a la parcela sellada SHOPPING para que no quede en 0, pero NO es el conteo real (un mall de 40 locales sigue mostrando 1). Sin solución gratuita conocida; requeriría una fuente paga o carga manual (candidato a un flujo de asistencia humana como el de habitaciones de hotel). |
...
| DXFExport | `dxf_export` | **Exporta el relevamiento a DXF (AutoCAD) con las UF de vivienda.** **DXF y no DWG a propósito:** el `.dwg` es formato cerrado de Autodesk y ninguna librería libre lo escribe; el DXF es el formato de intercambio del propio AutoCAD, que lo abre nativo y lo guarda como `.dwg` con "Guardar como" (si alguna vez piden el `.dwg` literal, el paso que falta es ODA File Converter sobre este mismo archivo). **Usa `ezdxf`, NO el driver DXF de GDAL** aunque GDAL ya esté instalado: el driver de OGR **no escribe atributos** (falla con `FieldError` al agregar un campo) ni textos — daría polígonos mudos, sin la UF, que es justo el dato a entregar. Capas separadas para prender/apagar: `PARCELAS` (polilínea cerrada del lote) · `UF_VIVIENDA` · `UF_COMERCIO` · `DIRECCION` (**sólo el número**: el nombre de calle se repite en toda la cuadra y no entra en un lote de 10 m de frente — la dirección completa viaja en los atributos) · `TIPO_EDIFICACION` (taxonomía del cliente, reusa `_tipo_edificacion` de la web) · `DATOS` (bloque `PARCELA_INFO` con **atributos invisibles** — 16 campos consultables desde la ventana de propiedades de AutoCAD sin ensuciar el dibujo) · `_METADATOS` (EPSG + survey + fecha + totales, para que el archivo se pueda auditar). **Coordenadas:** la DB está en 4326 (grados) y un CAD trabaja en metros ⇒ proyecta a UTM con `geo.utm_epsg` (huso autodetectado, válido en todo el planeta); en Brasil rotula el EPSG como **SIRGAS 2000** (319xx), que es numéricamente **idéntico** a WGS 84 UTM (verificado: diferencia 0,000 m) pero es la etiqueta que espera un proyectista brasilero. `epsg` en el input fuerza otro. El número **estimado** de `NumeroEstimator` sale **entre paréntesis**, no con `≈`: las fuentes SHX de AutoCAD no traen ese glifo. Altura de texto **escalada al ancho real del lote** (piso 0,35) y paso entre renglones sobre la altura real de cada uno — con altura fija los rótulos de los lotes angostos de VG se montaban entre sí. **El TOTAL sigue el criterio de la web/CSV, no la suma cruda**: las parcelas agrupadas en un `establecimiento` (fábrica/colegio sobre N lotes) NO se suman una por una — la entidad cuenta una vez, por la UF de su miembro más desarrollado (`SUM` de `establecimientos`). Sin esa guarda el DXF decía 885/1223 contra los 875/999 del KPI de la web, o sea el entregable contradecía a la app. El **rótulo de cada parcela sí muestra su propia UF** (es el dato de esa parcela) y el bloque trae el atributo `ESTABLECIMIENTO` para que se entienda por qué sumar los rótulos no cierra. Con `max_parcelas` (muestras) el ajuste **no** se aplica: el recorte no tiene por qué contener a todos los miembros. Sólo lectura: no toca la DB. Es **el DXF de los países cuyo cliente NO tiene estándar de planos propio** (hoy Argentina): `GET /api/surveys/{sid}/export/dxf` despacha a este agente sólo si el perfil del país no define `plano`, y si lo define sale `DXFEntrega` en su lugar — **un solo botón, el formato lo decide el perfil** (antes convivían dos y había que saber cuál pedir). Botón **"⬇ DXF (AutoCAD)"** en el detalle del survey, **visible también en la vista cliente** junto a "⬇ CSV Operadora"; genera on-the-fly en un temporal que se borra al terminar de servirse. Sigue disponible para cualquier país desde la CLI (`python -m scrapitero.rpc.dxf_export`), que es la herramienta interna de mirar un relevamiento. VG: 567 parcelas, 875 UF vivienda / 999 UF comercio, EPSG:31981. |
| DXFEntrega | `dxf_entrega` | **DXF con el estándar de planos del cliente (BL).** **Es el DXF que sale en Brasil**: desde el 20-ago-2026 `GET /api/surveys/{sid}/export/dxf` despacha acá cuando el perfil del país define `plano` (`_EXPORT_PERFILES_CLIENTE`, hoy sólo `BRA`), y a `DXFExport` cuando no. Aquél dibuja para nosotros (polígono de parcela + textos legibles al lado), éste dibuja **para ellos**, replicando `plano ejemplo de entrega.dwg` (recibido 18-ago-2026, carpeta Drive `BL_Compartido`). **Cómo plasma la info, que es lo opuesto a lo nuestro:** un **INSERT de bloque por inmueble** (`SDU` unifamiliar / `MDU` edificio), no un polígono con textos; **una sola etiqueta visible** por inmueble (`N_C1_3_TP_QT`: `350R` = nº 350 residencial 1 HP · `25R-2` = 2 HPs · `S/NC` sin número comercial · `VAZ` lote vacío) y los otros 14 atributos **invisibles**, consultables desde propiedades de AutoCAD; el tipo **además** en la capa (`SDU_RES`/`SDU_COM`/`SDU_ESP`/`SDU_VAZ`, `MDU_*`). **El mapa base es del cliente**: `QUADRA` y `MEIOFIO` salen del MUB municipal en `mapa_base_cliente` (mig. 062), no de nuestro catastro — con nuestros polígonos las manzanas no calzan contra el resto de sus planos. Recorte con `ST_Intersection` y no sólo `&&`: el operador de bbox traía enteras avenidas de 2,7 km que apenas rozaban la esquina. **Escala:** modelspace 1:1, **1 unidad = 1 metro**, UTM absoluto, sin paperspace ni cajetín (los dos `Layout` del ejemplo están vacíos). Alturas en metros del propio ejemplo: 1,5 etiqueta · 2,0 datos · 1,0 secundarios · 25–30 títulos. **R2000**, la versión de ellos. **Los bloques se importan de una plantilla** (`assets/plantilla_entrega_bl.dxf`, 53 KB, generada por `scripts/generar_plantilla_entrega.py`) y no se definen en código: sus símbolos traen geometría dibujada (la ficha del MDU es marco con ARC+LINE+LWPOLYLINE+3 TEXT) que redibujada a mano no daría lo mismo. **El estándar se extrajo del DWG con `dwgread -O JSON`, NO del DXF convertido**: `dwg2dxf` de LibreDWG es fiel con geometría (19.831 de 19.831 polilíneas del mapa base) pero **pierde INSERTs** (24 SDU donde el DWG tiene 391) y **descarta el bit de invisibilidad** — un estándar leído del DXF habría salido con los 15 atributos dibujados encima del plano. **Un bloque por EDIFICACIÓN, no por lote** (20-ago-2026): la parcela con vivienda y comercio sale como **dos** bloques, uno `SDU_COM` y otro `SDU_RES`, con el comercio en la mitad del lote más cercana al eje de la calle (`_puntos_para_dos` parte el polígono por su eje corto y toma un punto representativo de cada mitad, nunca el centroide). Es la convención del propio cliente, leída del DWG con `dwgread -O JSON`: **46 de sus 186 números aparecen repetidos**, con tipos distintos en el mismo número (`78` → `C,R,R,R` · `114` → `R,C,R-2`). Antes, con un bloque por lote, VG entregaba **78 comercios rotulados como residenciales y 30 viviendas como comerciales**. **El desglose sale de `uf_vivienda`/`uf_comercio` de la parcela y NO de `parcela_unidades`** — guarda contra perder HPs: el Hotel Portal da Amazônia tiene 72 UF de Cadastur y 2 unidades en el BCI, repartir por unidades le borraría 70 habitaciones. **Sólo se parte a nivel SDU**: la ficha `MDU` describe un edificio (lleva `HP_TOTAL`/`QTD_ANDARES`, no `TP`) y su tipo sigue viajando en la capa. VG: 534 → **602 SDU** + 33 MDU, con los 2.108 HP intactos. **Corte SDU/MDU sobre UF TOTAL (viv+com), no vivienda**: con vivienda sola el Shopping Várzea Grande (385 UF de comercio) y un hotel de 72 unidades salían como SDU con etiquetas tipo `(388)C-72`. Default `mdu_min_uf=6` → VG da 534 SDU + 33 MDU. **La hoja se dimensiona por los puntos de rotulado, no por los polígonos**: una parcela de `AV. GOV. JOAO PONCE DE ARRUDA` sin número tiene **4.606.312 m²** (190× la siguiente) y estiraba el dibujo a 3.845×5.244 m para un relevamiento de 1.708×1.619, con un triángulo cruzando el plano y `LIMITE` de 5,31 km². Se avisa por `geometrias_rotas` pero **no se descarta**: el dato de la parcela es real, lo roto es el polígono. `LIMITE` = envolvente **cóncava** (`ratio=0.12`, +30 m) — la convexa encerraba kilómetros no relevados. **Los rótulos de calle salen de las parcelas, no de `logradouros`**: esa tabla tiene geometría pero 49 de 3.522 filas con nombre y **ninguna** con CEP; el eje se deduce por componente principal de las parcelas de cada calle. Y **no se filtra por `region_id`** (el survey es `zona-varzea-grande-actualizacion` y los logradouros `vg-mt-br`: filtrar daba 0 calles — el mismo modo de falla que hace que los overrides no se hereden). **`TIPOIMOVEL` sale VACÍO** (20-ago-2026), como en el plano del cliente: leído del DWG original viene presente pero vacío en **0/388** SDU, igual que `NOME`/`CLASSE_SOCIAL`/`CODGED`. Llenarlo con `_tipo_edificacion` contradecía la etiqueta visible en **92 de los 534 bloques** de VG. **`NumeroEstimator` no es la última opción para el número**: si el lote sale sin numerar (`0`) y un **lindero comparte el nombre del establecimiento** (`_numeros_de_lindero`: se tocan a ≤5 m, los dos con `complemento`, nombre significativo de ≥10 caracteres sacando la jerga catastral tipo `AREA REMANESCENTE`, y si dos linderos discrepan no se elige ninguno), se usa **el número real del lindero**, sin paréntesis — el hotel de 72 habitaciones salía rotulado `(388)` teniendo el 400 al lado. **Lo que NO se llena, a propósito:** `CODLOG` (viene de un CSV del cliente vía `codlog_csv`; sin él, vacío — en su ejemplo está **383/388** lleno, o sea que lo exigen) · `ID` de célula (parámetro `celula_id`, p.ej. `VAZ049`) · `CODGED`/`NOME`/`CLASSE_SOCIAL`/`TIPOIMOVEL` vacíos, **como en su propio ejemplo** (0/24 los cuatro) · `QTD_ANDARES` del MDU: manda `pisos_estimados_max` y, como está **vacío en las 567 parcelas de VG**, cae a **`parcela_altura.pisos_satelital`** (AlturaFetcher) — cubre 26 de las 33 fichas; sin ninguno de los dos sale vacío, no se inventa altura · `POSTE`/`DROP`/`FLY_TAP`/`HCSDU`/`HCMDU` son diseño de red, no los relevamos y no se inventan (capas y bloques quedan definidos, vacíos). Botón **"⬇ DXF Entrega (cliente)"** — el único botón DXF del survey, etiquetado con la `etiqueta` del perfil (`GET /api/surveys/{sid}/export/dxf?celula_id=&codlog_csv=`, los dos parámetros se ignoran si el país no tiene `plano`); pide el ID de célula porque lo asigna el cliente por zona. Sólo lectura. VG: 534 SDU + 33 MDU, 2.108 HPs, 31 logradouros, 112 manzanas + 136 cordones, EPSG:31981. |
...
1. ARBACartoFetcher    → parcelas con geometría + UF/cocheras (requiere JSESSIONID vigente)
   ...
   Es REANUDABLE (`rehacer=false` por default): saltea las que ya tienen nomenclatura, así
   un corte se retoma sin re-pagarle a Google el reverse. `rehacer=true` re-barre TODO.
2. FootprintFetcher    → footprints (Google Open Buildings, GRATIS; OSM es inservible en el
                         conurbano: 2.872 vs 4 edificios medidos en Hurlingham)
3. AddressResolver     → completar direcciones faltantes
4. OverturePlacesFetcher + **OsmPoiFetcher** + ParcelaCategoria → comercios reales y tipo de
                         edificación (GRATIS). `OsmPoiFetcher` va en el medio: trae las
                         **estaciones de servicio**, que Overture NO publica en Argentina
                         (0 en 218 POIs de Malvinas; OSM tenía 2 en la zona).
5. NumeroEstimator     → número de puerta donde el catastro no lo trae (eje OSM; gratis)
6. ShoppingFetcher     → `fuentes=['osm','shoppings_ar']` (GRATIS). `shoppings_ar` NO está en
                         el default y es la fuente buena en AR: da el nombre comercial real.
7. HotelFetcher        → `fuentes=['google']` (PAGO, ~USD 2 en una zona chica). Es la ÚNICA
                         fuente de descubrimiento en el conurbano: no hay Cadastur argentino
                         y Overture no trae alojamientos en AR (0 en 355 POIs medidos).
                         Sumar `'osm'` es gratis pero aporta poco.
8. IncidenciasReporter → lo que sólo un humano cierra (UF sin declarar, hoteles sin
                         habitaciones, números faltantes)
9. RelevamientoCSV / CSV Operadora (perfil ARG) / DXF → entregable
```

**No corras `AlturaFetcher` en PBA:** su criterio compara contra el área construida del
catastro y **ARBA no la publica** ⇒ marca `sin_declarar` en TODAS las parcelas. Para "hay
construcción y 0 UF" ya está el tipo `uf_sin_declarar` de `IncidenciasReporter`, que en
Argentina usa la **huella de los footprints** en vez del área construida. Lo mismo hace la
etiqueta de tipo de edificación desde la mig. 058 (`parcelas.huella_m2`): en PBA la huella
satelital es *la* señal de "acá hay algo construido". **Corré `FootprintFetcher` antes que
nada en un relevamiento argentino** — sin él las guardas de POI se saltean con WARNING y
medio relevamiento sale rotulado LOTE VAZIO.

**`UsoClassifier` dejó de ser necesario en PBA** (2026-08-11): su motivo era que el uso
quedaba NULL, y ahora `ARBACartoFetcher` deduce `residencial` de sus propias UF y
`OverturePlacesFetcher` marca `comercial`/`mixto` donde hay comercios reales — gratis y sin
Google Places. Corrélo sólo si querés contrastar con la señal paga de Places; su sello
`clasificador` está en `USO_FUENTES_PROTEGIDAS`, así que no lo pisa un catastro después.

**Regla PBA — uso_principal SIEMPRE se clasifica:** PBA no tiene fuente nativa de uso
(a diferencia de Brasil=BCI y Salta=CPUA/SIGSA). Cómo se reparte hoy:
...
- **Con UF > 0 y ningún comercio conocido, ARBACartoFetcher sella `residencial`** (misma
  regla que el BCI en Brasil: la única señal es el conteo). Con **0 UF no infiere nada** —
  puede ser un baldío o un lote que carto no devolvió, y desde ahí no se distinguen. Sin
  esto el uso quedaba NULL en **592 de 619** parcelas de Malvinas (96%): sin color en el
  mapa y sin uso en el CSV, con 628 viviendas ya contadas.
- **`OverturePlacesFetcher` + `OsmPoiFetcher` son los que marcan comercio** (gratis, las dos)
  y **mandan sobre el catastro**: el sello `poi` está en `USO_FUENTES_PROTEGIDAS`, así que un
  `residencial` inferido del conteo nunca degrada el `mixto`/`comercial` que puso quien sí
  vio los comercios. El uso sale de **las dos UF de la fila**, no del `uso_principal` previo
  — con el CASE viejo, que sólo ascendía a `mixto` si la parcela ya venía `residencial`, en
  Hurlingham **27 de 28** parcelas con vivienda Y comercio terminaron `comercial` y ninguna
  mixta. **Correr las dos**: Overture sola dejaba 32 negocios afuera en Malvinas.
- ⚠ **El comercio se DESCUENTA del conteo de ARBA, no se suma encima.** ARBA da el CUÁNTAS
  sin el destino, así que un comercio confirmado no es una unidad nueva: es una de esas
  mismas, mal rotulada como vivienda. `uf_vivienda = max(uf_catastro − comercios, 0)`,
  `UF total = max(uf_catastro, comercios)`. Sumando —como hacía la ruta de Overture hasta
  ago-2026— el shopping Terrazas de Mayo salía con **«1 vivienda + 32 comercios»**.
- ⚠ **Lo que el descuento NO arregla, y es decisión del cliente:** donde no hay ninguna señal
  de comercio, la unidad sin destino se sigue contando como vivienda. En Malvinas son
  **1.875 de 1.974 parcelas con exactamente "1 vivienda"** puesta por default. Hay que
  acordar si eso se cuenta como vivienda o se deja sin clasificar.
- **`UsoClassifier` (Google Places, pago) quedó como opcional** — ver arriba.
- **Todo esto depende de ARBACartoFetcher:** si las parcelas entraron solo por IDERA
  (`arba_idera`, geometría sin UF), no hay UF que repartir. Correrlo (con JSESSIONID) antes.
- Requiere `GOOGLE_MAPS_API_KEY`. Para el **conteo real de comercios** usar
  **`OverturePlacesFetcher`** (gratis) y después `ParcelaCategoria`; `GooglePlacesFetcher`
  queda como opción paga sólo si hace falta contrastar. En Hurlingham Overture aportó 53 UF
  de comercio en 28 parcelas y 78 etiquetas de la taxonomía, sin gastar un peso.
...
⚠ **IDERA trunca en 1000 features y NO avisa** (ignora `count=50000` y devuelve 1000 con
HTTP 200). Como el chequeo de truncamiento comparaba contra el count **pedido**, nunca
saltaba: en Malvinas Argentinas quedaron **481 parcelas de ~1000**, con el 38% de la
superficie vacía y 2.032 edificios adentro — un relevamiento a medias que se veía completo.
Se pagina con `startIndex` hasta que una página vuelva incompleta. Ver
[[project_fuentes_fallo_silencioso]].

⚠ **La identidad de una parcela al re-correr es la COORDENADA, no el `cca_code`:** hay
inscripciones repartidas en más de un polígono (José León Suárez 1397 y 1466 comparten
`133050J…0160000001000`, "Manzana 16 Parcela 1" las dos, a 70 m). Deduplicar por CCA las
fusionaría y perdería una parcela real. Tampoco sirve filtrar por `fuente_parcela` para
encontrar la existente: el enriquecimiento de carto reescribe ese campo a `arba_carto`, así
que toda parcela ya procesada dejaba de matchear y se re-insertaba (2.678 filas, 620
duplicadas). El match es por `survey_id` + centroide (±0,00005°).
...
- **La taxonomía del cliente (`TIPOS_EDIFICACION`, `descripcion_uso`) NO va por `i18n.js` sino
  ...
  ⚠ **La traducción se valida contra QUIÉN cae en la etiqueta, no contra el diccionario.**
  `LANCHONETE` estaba traducida **«ROTISERÍA»** y está mal: una *lanchonete* es el mostrador
  donde se come un sándwich o se toma un café, no el local que vende comida hecha para llevar.
  Los rubros que alimentan la etiqueta —`fast_food_restaurant`, `cafe`, `coffee_shop`,
  `casual_eatery`, `food_service`, `amenity=ice_cream`— dieron en Malvinas Burger King, KFC,
  Starbucks, Havanna, Guapaletas y el patio de comidas del shopping: **ni una rotisería en 15**.
  Corregida a **«CAFETERÍA / COMIDAS RÁPIDAS»** (13-ago-2026). El costo de una traducción mala
  no es cosmético: el operador compara contra Google Maps, ve «ROTISERÍA» en la vereda de un
  McDonald's y da por malo el relevamiento entero. Al sumar una etiqueta nueva, mirar la lista
  real de POIs que cayeron en ella antes de elegir la palabra.
...
⚠ **Fuera de Brasil hay etiquetas EXTRA** (`TIPOS_EDIFICACION_EXTRA`, 13-ago-2026). Hoy sólo
**PRAÇA (E) → «PLAZA»**. `TIPOS_EDIFICACION` es contrato del cliente brasilero y no se toca:
agregarle un ítem se lo mete en el desplegable y en el CSV de Várzea Grande, donde nadie lo
pidió. Pero un relevamiento argentino tiene usos que la lista no cubre y forzarlos a la
etiqueta más parecida miente — una plaza pública no es `LOTE VAZIO` (nadie la va a construir,
no tiene dueño) ni `ESTACIONAMENTO`. **El país lo decide el `survey_id`, NO el `?lang=`**:
mirar VG en español es preferencia de pantalla, no habilita etiquetas nuevas. `_TIPO_CATEGORIA`
incluye las extra o los endpoints las rebotan al validar. Tabla en `docs/TIPOS_PROPIEDAD.md`.

⚠ **Los POIs se dibujan en el mapa, iguales a una parcela** (13-ago-2026,
`GET /api/surveys/{sid}/pois` + `loadPois`). `establecimientos_poi` en **su** coordenada,
mismo `circleMarker` que la parcela (radio 7, anillo `#0f172a` 1.5, `fillOpacity` .9), en el
mapa **directamente y no en un overlay**. Existen porque un POI dentro de un lote grande es
invisible: la **estación de servicio Puma** cae adentro de los 131.620 m² de Terrazas de Mayo,
así que el mapa dibujaba un círculo —el del shopping— y la estación, que está pegada al
McDonald's y se ve desde la ruta, no aparecía en ninguna parte aunque estuviera cargada y
contada. Mismo caso: la cancha adentro del predio de un club, la escuela adentro de un campus.
**El estilo es decisión de producto (Jazmín), no accidente**: la primera versión los hacía
chicos, con borde blanco y en una capa apagada, y se leían como otra clase de cosa —
*"un punto es un punto, lo único que cambia es el color, el nombre y la taxonomía"*. Si el
marcador de parcela cambia en `loadParcelasMap`, hay que cambiar éste en el mismo turno.
**Lo único que no llevan es el número de UF al costado**: su UF está contada en la del lote
que los contiene (el popup dice cuál), y ponerles número propio la contaría dos veces a ojo.
**Excluye el POI que una guarda desvinculó a propósito** (`vinculo_resuelto` + `parcela_id`
NULL): ésos están mal ubicados a sabiendas y dibujarlos vuelve a afirmar la coordenada que la
guarda descartó.

⚠ **Un lote con POI CONTENEDOR devuelve UN punto, no N** (`_POI_CONTENEDOR` = SHOPPING ·
UNIVERSIDADE/FACULDADE · HOSPITAL PÚBLICO/PARTICULAR · INSTITUICAO ESPORTIVA, en ese orden).
Sin esto el shopping quedaba con **19 puntos apilados** y el campus de la UNGS con **21**: sus
locales y dependencias no son cosas separadas, están adentro. El contenedor viaja con
`contiene` (los nombres que absorbió) y el popup los lista completos. Malvinas: **111 → 73
puntos, 39 colapsados**. Detalles que costaron una iteración cada uno:

- **El orden de `_POI_CONTENEDOR` importa.** El lote de Terrazas de Mayo tiene un SHOPPING y
  además dos `INSTITUICAO ESPORTIVA` (Flow Sports, LifeCenter Fitness) que son **locales
  adentro** del shopping, no el continente.
- **Con varios candidatos gana el de OSM**, no el más cercano al centroide ni el primero
  alfabético. OSM mapea el shopping y el campus como **polígono** y su punto es el centro del
  predio; Overture pone un punto por local (lo mismo que ya documenta `OsmPoiFetcher` sobre
  por qué *Terrazas de Mayo* aparece a 175 m entre las dos fuentes). Alfabético elegía
  «Cátedra Ecología Huerta Agroecológica UNGS» y por centroide salía «Frente NODOCENTE UNGS».
- **Si todos los candidatos son de Overture NO se elige ninguno**: se arma un punto
  **sintético del lote** (nombre = su dirección, posición = su centroide, etiqueta = la del
  contenedor, `fuente='lote'`). En el campus de la UNGS los 13 `college_university` son
  dependencias sueltas y «Universidad Nacional de General Sarmiento» está entre ellas **sin
  ninguna marca que la distinga**, ni por nombre ni por posición (60 a 142 m del centroide,
  sin orden). Poner una a dedo rotularía el predio entero con un nombre equivocado.
- **`_POI_NO_COLAPSA` (hoy POSTO DE GASOLINA) conserva su punto** y no entra en `contiene`:
  es un edificio aparte sobre el mismo lote, se ve desde la calle y no es inquilino de nadie.
  Es literalmente el caso que el operador reportó como faltante.
- ⚠ **El colapso es por LOTE, no por edificio.** Probado con `footprints_revision` y **no
  sirve**: el punto de Overture apunta al playón, así que Burger King, Starbucks, KFC y Questa
  Pizza dan "fuera del edificio" igual que la Puma. Ese test mide la precisión de la
  coordenada, no si algo está adentro del shopping. No volver a intentarlo por ahí.

⚠ **La lista 🏪 del popup va COMPLETA, sin cortar** (13-ago-2026). Estaba en `slice(0, 8)` +
«…» y eso hacía parecer *faltante* algo que estaba cargado y contado: la **Puma de Terrazas de
Mayo** es la **26ª de 35** en orden alfabético (`array_agg(... ORDER BY c.nombre)`), así que la
estación que se ve desde la ruta no aparecía por ningún lado y el operador la reportó como no
relevada. En un lote catastral grande —el del shopping son 131.620 m² con todo adentro— el
corte esconde justo lo que se va a buscar. El tope ahora es visual: `maxHeight: 360` en el
`bindPopup`, que hace scrollear el globo en vez de estirarlo (sin eso los botones de comentar
y 📌 quedaban fuera de pantalla). ⚠ El rótulo sigue diciendo **"(Google)"** y ya no es cierto
—hoy esos comercios vienen de Overture y OSM—; para arreglarlo hay que subir `comercios.source`
por la API, no basta con cambiar el texto.

⚠ **"Sin construir" son CUATRO señales, no una** (ago-2026). Sale LOTE VAZIO sólo si fallan
todas: `area_m2_construida` (BCI) · `uf_vivienda` · `uf_comercio` · **`parcelas.huella_m2`**
(m² construidos vistos por satélite, mig. 058, materializada por `FootprintFetcher`). Cada una
tapa un agujero de la anterior **en Argentina**, donde ARBA no publica área construida: sin
`uf_comercio` la parcela puramente comercial se contradecía sola en el popup —«🏢 LOTE
BALDÍO» arriba y «Com: ≈ 1» abajo— y sin `huella_m2` la parcela sin uso clasificado y sin UF
—lo normal en PBA— salía baldía teniendo casas adentro. Medido el 13-ago-2026 en Malvinas +
Hurlingham: **59 parcelas rotuladas baldío, 52 con edificios adentro**; el caso testigo es
*José María Márquez 1245*, 4.085 m² con 1.175 m² construidos. **`huella_m2` NULL ≠ 0**: NULL
es "el relevamiento no bajó footprints" y 0 es "los bajó y no hay nada" — la regla pregunta
`> 0`, que es falso para los dos, así que un survey sin footprints conserva el comportamiento
viejo (misma distinción que las guardas de `OverturePlacesFetcher`). **`uso='vacante'` gana
igual**: si el catastro AFIRMA que está vacío se le cree aunque el satélite vea un edificio —
esa discrepancia es `uf_sin_declarar`/`AlturaFetcher`, no de la etiqueta. Quien agregue un
call site de `_tipo_edificacion` tiene que pasar `uf_c` y `huella`, o la parcela argentina
vuelve a salir baldía en silencio.
...
- **`_EXPORT_PERFILES_CLIENTE`** (salida): formato de entrega propio de cada cliente. Definidos
  ...
  **`cod_operadora`**, y una cuarta **opcional `plano`** (hoy sólo `BRA`): el estándar de planos
  del cliente. Con `plano` definido, `GET /export/dxf` sale en **su** formato (`DXFEntrega`,
  bloques SDU/MDU sobre su mapa base) y el front pide antes el ID de célula; sin `plano`, sale el
  dibujo interno de `DXFExport`. **Es un solo botón en los dos casos.** El front decide con
  `export_cliente` / `export_plano` del payload del survey (no con `country_code === 'BRA'`), y
  `GET /api/surveys/{id}/export/perfiles` dice qué aplica. **El CSV genérico no depende del país.**
