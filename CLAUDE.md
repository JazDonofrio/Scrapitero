...
| BaselineInterp | `baseline_interp` | **Desapila las direcciones del relevamiento anterior que cayeron en un mismo punto.** geocodebr devuelve UN punto aproximado (`g:numero_aproximado`) para los números que no están en el CNEFE → muchos números de la misma calle se apilan en una coordenada y el mapa los muestra como un solo marcador. Estrategia definitiva en 3 niveles, **mínimo costo**: (1) **interpolación gratis** por calle usando los números **exactos** (`g:numero`/mapbox/google, coordenada única) como **anclas** — interpola a tramos dentro del rango y extiende con la pendiente global afuera (marca `geocode_source='interp'`); (2) calles **sin ≥2 anclas** → **distribuye sobre la geometría de la calle de OSM** (Overpass, **1 consulta por calle** filtrando por token del nombre + match por núcleo fuzzy ≥0.82; tope 80 calles; `osm_interp`); (3) lo que no matchea queda en su punto. **Corre solo** como pase final de **BaselineGeocoder** (best-effort) y por RPC. El match de nombre usa el **núcleo** de la calle (sin tipo de vía ni basura tipo "(LOT …)"/complemento) → tolera AV/R/TV y abreviaturas. |
