...
**`parcela_uf_manual`** (mig. 046) · `numero` → `parcelas.numero_estimado`
(`metodo='manual'`, conf. 1,0) + **`parcela_numero_manual`** (mig. 051). Endpoints: `GET /api/surveys/{sid}/incidencias?estado=&tipo=`
...
**Número de puerta estimado (🔢):** las parcelas que el catastro dejó sin altura (`numero` en `0`
o vacío) reciben un número **inferido** por `NumeroEstimator` (ver su fila arriba). Se muestra
siempre **marcado**: en el popup del mapa como **"≈ N est."** (ámbar, con tooltip del método y la
confianza) y en el CSV en columnas propias (**Número estimado / método / confianza**). Nunca se
mezcla con la dirección ni entra al **CSV Operadora** — `parcelas.numero` es dato del municipio y
el estimado vive en `numero_estimado*` (mig. 050).
